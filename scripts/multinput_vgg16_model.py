...
def _parse_data(x_sat, x_street, y):
  ##This processes the image data 
  sat_image = tf.io.read_file(x_sat)
  sat_image = tf.image.decode_jpeg(sat_image, channels=3)
  sat_image = tf.cast(sat_image, tf.float32)
  sat_image = (sat_image/127.5) - 1
  sat_image = tf.image.resize(sat_image, (IMG_SIZE, IMG_SIZE))

  street_image = tf.io.read_file(x_street)
  street_image = tf.image.decode_jpeg(street_image, channels=3)
  street_image = tf.cast(street_image, tf.float32)
  street_image = (street_image/127.5) - 1
  street_image = tf.image.resize(street_image, (IMG_SIZE, IMG_SIZE))
  ...
  return (sat_image, street_image), y
 
def _input_fn(x_sat, x_street, y, data_size=street_data_size):
  ## Reference : https://www.tensorflow.org/api_docs/python/tf/data/Dataset 
  
  #According to website: "Creates a Dataset whose elements are slices of the given tensors."
  ds=tf.data.Dataset.from_tensor_slices((x_sat,x_street, y))
...
train_ds = _input_fn(sat_x_train, street_x_train, sat_y_train)
test_ds =  _input_fn(sat_x_test, street_x_test, sat_y_test)
...
history = sat_street_model.fit(train_ds, epochs=100, steps_per_epoch=2, validation_steps=2, validation_data=test_ds)
validation_steps = 20
loss0,accuracy0 = sat_street_model.evaluate(test_ds, steps = validation_steps)
